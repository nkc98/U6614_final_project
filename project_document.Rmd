...
We evaluate the effect of Medicaid expansion on drug overdose deaths in Appalachia using both a difference-in-differences approach and an event study approach. The former allows us to establish a single average causal treatment effect estimate over the five years after Medicaid was expanded. The latter allows us to examine year-by-year treatment effects relative to the year prior to Medicaid expansion. 

Furthermore, we look for heterogeneous treatment effects between "high" and "low risk" counties in both approaches. 
...
names(did_models) <- c("All Counties")
...
```{r fig3, fig.cap = "Medicaid Expansion's Effect on Drug Overdose Death Rates in All Appalachian Counties"}
iplot(
  ES_mod,
  pt.join = TRUE,
  ci.join = TRUE,
