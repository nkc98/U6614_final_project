...
```{r fig1, fig.cap="Map of Medicaid Expansion Status Across Appalachian Counties", fig.width=5, fig.height=3.5}
expansion_map
```
...
did_estimation_results <- modelsummary(did_models,
  ...
  column_spec(column = 2:4, width = "2in") %>% 
  kable_styling(latex_options = c("hold_position", "scale_down")) 

coef <- round(did_models$`All Counties`$coefficients[1], 2)
```
...
```{r fig4, fig.cap = "Medicaid Expansion's Effect on Drug Overdose Death Rates in High Risk Counties", fig.width=6, fig.height=4}
iplot(
  ES_mod_high_risk,
  pt.join = TRUE,
  ci.join = TRUE,
...
```{r fig5, fig.cap = "Medicaid Expansion's Effect on Drug Overdose Death Rates in Low Risk Counties", fig.width=6, fig.height=4}
iplot(
  ES_mod_low_risk,
  pt.join = TRUE,
  ci.join = TRUE,
...
ES_estimation_results <- modelsummary(ES_models,
                                      ...
                                      'White pop. share, Hispanic pop. share, and Asian pop. share.',
                                      'Control covariates include county-level poverty rate, median age, male pop. share, Black pop. share,')) %>% 
  column_spec(column = 2:4, width = "1.5in") %>% 
  kable_styling(latex_options = "hold_position")
```
