...
describe(pre_2014_expansion_county_panel[, c('med_income', 'med_age', 'black_share', 'male_share')], fast=TRUE)
...
```{r}
describe(post_2014_expansion_county_panel[, c('med_income', 'med_age', 'black_share', 'male_share')], fast=TRUE)
```

### Pre-Expansion Descriptive Statistics in Non-Expansion States

```{r}
describe(pre_2014_non_expansion_county_panel[, c('med_income', 'med_age', 'black_share', 'male_share')], fast=TRUE)
```

### Post-Expansion Descriptive Statistics in Non-Expansion States

```{r}
describe(post_2014_non_expansion_county_panel[, c('med_income', 'med_age', 'black_share', 'male_share')], fast=TRUE)
```

[It is noteworthy that the mean of black share in non-expansion states is much higher than that in expansion-states regardless of years. The means of black share in non-expansion states for both pre-expansion and post-expansion are both over 10%, but the means of black share in expansion states for both pre and post-expansion are 2.19% and 2.21%, respectively. Moreover, the range of median income for non-expansion states is much higher than that in expansion states regardless of years. Specifically, the rage of median income in non-expansion states for pre and post expansion are 65380 and 62585, respectively, and the range of median income in expansion states for pre and post expansion are 45680 and 47531. it means that the gap between the rich counties and the poor counties is larger in non-expansion states than in expansion states.]
...
Additionally, we include the term $Post~Expansion_{t}$, which indicates (0-1) observations measured prior to 2014 Medicaid Expansion, to control for unobserved county-invariant factors that might have changed between the pre- and post-expansion time periods (i.e., "time fixed effects"). Such factors would include events such as periodic economic shocks that affect the entire Appalachian region in certain years, which potentially affect the rate of overdose deaths. We further include an array of county-specific dummy variables $\Gamma_{i}$ that control for unobserved time-invariant factors that are specific to individuals counties (i.e., "county fixed effects"). An example of one such factor would be if, throughout the entire 2012-2015 period, a specific county had its own drug treatment program that reduced drug overdose deaths compared to other counties, all else equal.
